/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Manages Products, Ads/Banners, Inquiries, Settings and Page Content.
   Everything persists via window.Store and reflects on the public site
   immediately; once Firebase is configured it also mirrors to Firestore.

   AUTH — two modes:
     • Firebase configured -> real Firebase Auth. Credentials live in the
       Firebase console. This is the ONLY mode you should launch with.
     • Firebase blank      -> the DEMO gate below (sessionStorage). There is NO
       real security here. It exists so the panel is usable before the backend
       is set up; in this mode every edit stays in your own browser and is not
       public, so the blast radius is your own localStorage.
   >>> Configure Firebase (see FIREBASE_SETUP.md) before going live, and the
       demo gate is bypassed automatically.
   ========================================================================== */
...
(function () {
  ...
  function wireLogin() {
    ...
    const cloud = !!(window.SGVCloud && window.SGVCloud.enabled);
    let entered = false;

    if (cloud) {
      // Auto-enter if a Firebase session is already active (persists across reloads).
      window.SGVCloud.onAuth((user) => { if (user) enter(); });
    } else if (sessionStorage.getItem("sgv_admin") === "1") {
      enter();
    }
    ...
    form && form.addEventListener("submit", async (e) => {
      ...
      if (cloud) {
        try {
          await window.SGVCloud.signIn(email, pass);   // onAuth handler calls enter()
        } catch (e2) {
      ...
      if (email === DEMO.email && pass === DEMO.pass) {
        sessionStorage.setItem("sgv_admin", "1");
        enter();
      } else {
        // Deliberately vague: don't advertise the demo credential on a page that
        // may already be public. It's documented in README.md instead.
        err.style.display = "block";
    ...
    $(".logout") && $(".logout").addEventListener("click", () => {
      sessionStorage.removeItem("sgv_admin");
      if (cloud) window.SGVCloud.signOut();
      entered = false;
    ...
    async function enter() {
      ...
      if (cloud) {
        try {
          if (await window.SGVCloud.hasData()) {
            await window.SGVCloud.pull(true);          // cloud is the source of truth
          } else {
            await window.SGVCloud.publishAll();         // first run: seed the cloud from local
  ...
  function loadBranding() {
    ...
    if (!form) return;
    const s = Store.getSettings();
    form.emblem.value = s.emblem || "assets/img/icons/emblem.svg";
