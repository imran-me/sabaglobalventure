...
(function () {
  ...
  /** Is this config value still an unfilled {CURLY} placeholder (or empty)? */
  function isUnset(v) {
  ...
  /** WhatsApp click-to-chat URL, or "#" while the number is unset.
   *
   *  IMPORTANT: do NOT build a link from an unfilled placeholder. Stripping the
   *  non-digits out of "{WHATSAPP_E164}" leaves "164", which yields the perfectly
   *  plausible-looking `https://wa.me/164` — a live link to somebody else. A dead
   *  link that is obviously dead is far safer than a wrong number that isn't.
   */
  function whatsappUrl(opts = {}) {
    if (isUnset(cfg.whatsapp)) { warnUnset("config.whatsapp"); return "#"; }
    const num = String(cfg.whatsapp).replace(/[^\d]/g, "");
    if (!num) { warnUnset("config.whatsapp"); return "#"; }
    const text = encodeURIComponent(buildMessage(opts));
    return `https://wa.me/${num}?text=${text}`;
  }

  /** Standard mailto: link, or "#" while the address is unset. */
  function mailtoUrl(opts = {}) {
    if (isUnset(cfg.email)) { warnUnset("config.email"); return "#"; }
  ...
  function wireDataAttrs(root = document) {
    root.querySelectorAll("[data-cta]").forEach((el) => {
      ...
      // Mark (don't hide) CTAs whose contact detail is still a placeholder, so
      // the gap is obvious in dev and in review rather than shipping a link to
      // the wrong number.
      const unset = url === "#";
      el.toggleAttribute("data-cta-unset", unset);
      if (unset) el.setAttribute("aria-disabled", "true");
  ...
  window.CTA = { isUnset, fillTemplate, buildMessage, whatsappUrl, mailtoUrl, gmailUrl, wireDataAttrs };
})();
