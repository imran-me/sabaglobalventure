/* ============================================================================
   ...
   read, and syncs that cache with Firestore.
     ...
     • Public pages  -> SGVCloud.pull()  loads published docs into the cache,
                        then main.js renders from it (so everyone sees the
                        latest admin edits — no more export-and-commit step).
     • Admin writes  -> store.js calls SGVCloud.mirror(...) on every save/delete,
                        pushing the change up to Firestore.
     • Admin login   -> SGVCloud.signIn() uses Firebase Auth so writes pass the
